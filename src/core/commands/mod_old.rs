...
fn fetch_accounts_with_usage(
    ...
    let cache_path = std::env::temp_dir().join("kiro-cli-auth-cache.db");
    ...
    let handles: Vec<_> = accounts.iter().map(|account| {
        ...
        thread::spawn(move || {
            // 先從快取讀取（樂觀鎖策略）
            let cached_info = if let Ok(cache) = AccountCache::new(&cache_path) {
                cache.get(&account.email).map(|(info, _)| info)
            } else {
                None
            };
...
pub fn cmd_list(fm: &FileManager) -> Result<()> {
    ...
    let results = fetch_accounts_with_usage(&accounts, &kiro_data, current_email.as_ref(), &cache);
...
pub fn cmd_refresh(fm: &FileManager, alias: Option<String>) -> Result<()> {
    let conn = fm.get_db_connection()?;
    let accounts = db::list_accounts(&conn)?;
    
    if accounts.is_empty() {
        println!("{}", ui::yellow("No accounts available"));
        return Ok(());
    }

    let aliases_to_refresh: Vec<String> = if let Some(a) = alias {
        vec![a]
    } else {
        accounts.iter().map(|a| a.alias.clone()).collect()
    };

    println!("{} Refreshing {} account(s)...\n", ui::cyan("→"), aliases_to_refresh.len());

    let mut success_count = 0;
    let mut failed_count = 0;

    for alias in &aliases_to_refresh {
        let account = match db::find_account(&conn, alias)? {
            Some(a) => a,
            None => {
                println!("{} {} - Not found", ui::red("✗"), ui::cyan(alias));
                failed_count += 1;
                continue;
            }
        };
        ...
        let snapshot_path = std::path::Path::new(&account.snapshot_path);
        if !snapshot_path.exists() {
            println!("{} {} - Snapshot not found", ui::red("✗"), ui::cyan(alias));
            failed_count += 1;
            continue;
        }

        // 嘗試刷新 token
        match extract_refresh_token(snapshot_path) {
            Ok(refresh_tok) => {
                // 判斷 provider
                let provider = extract_account_info(snapshot_path)
                    .ok()
                    .map(|(_, p)| p)
                    .unwrap_or_default();
                ...
                match refresh_result {
                    Ok(refresh_resp) => {
                        let new_refresh = refresh_resp.refresh_token.as_deref();
                        if let Err(e) = update_token(snapshot_path, &refresh_resp.access_token, new_refresh) {
                            println!("{} {} - Failed to update: {}", ui::red("✗"), ui::cyan(alias), e);
                            failed_count += 1;
                        } else {
                            // 更新快取
                            let cache_path = std::env::temp_dir().join("kiro-cli-auth-cache.db");
                            if let Ok(cache) = AccountCache::new(&cache_path) {
                                if let Ok(info) = get_account_info(&refresh_resp.access_token) {
                                    let _ = cache.set(account.email.clone(), info, refresh_resp.access_token);
                                }
                            }
                            println!("{} {} - Token refreshed", ui::green("✓"), ui::cyan(alias));
                            success_count += 1;
                        }
                    }
                    Err(e) => {
                        println!("{} {} - Refresh failed: {}", ui::red("✗"), ui::cyan(alias), e);
                        failed_count += 1;
                    }
                }
            }
            Err(e) => {
                println!("{} {} - No refresh token: {}", ui::red("✗"), ui::cyan(alias), e);
                failed_count += 1;
            }
        }
    }
    ...
    println!("\n{}", "─".repeat(70));
    println!("{} {}/{} accounts refreshed successfully", 
        ui::bold("Summary:"), 
        success_count, 
        aliases_to_refresh.len()
    );
    if failed_count > 0 {
        println!("{} {} accounts failed (may need re-login)", 
            ui::yellow("⚠"), 
            failed_count
        );
    }
